...
import { ExternalLink, Github, HardHat, Globe, Package, Cpu } from "lucide-react";
import { StarsBackground } from "@/components/ui/stars-background";
import { GsapText } from "@/components/animations";

// 分类图标：key 需要和 data/projects.ts 里的 category 保持一致
const categoryIcons: Record<string, React.ReactNode> = {
  engineering: <HardHat className="h-4 w-4" />,
  digital: <Globe className="h-4 w-4" />,
  "supply-chain": <Package className="h-4 w-4" />,
  product: <Cpu className="h-4 w-4" />,
};
...
export function Projects() {
  ...
  const categories = [
    ...
    { value: "engineering", label: "工程项目" },
    { value: "digital", label: "数字化与外贸" },
